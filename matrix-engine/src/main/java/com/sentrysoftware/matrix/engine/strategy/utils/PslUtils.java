...
public class PslUtils {
	...
	public static List<List<String>> ipmiTranslateFromWmi(@NonNull List<List<String>> wmiComputerSystem,
		...
		for (List<String> line : wmiNumericSensors) {
			...
			String sensorId = description.split("\\(\\)")[1];
 ...
 * 
 * 
	public static List<String> ipmiTranslateFromWmi(@NonNull List<List<String>> wmiComputerSystem,
			@NonNull List<List<String>> wmiNumericSensors,
			@NonNull List<List<String>> wmiDiscreteSensors) {
		List<String> ipmiTable = new ArrayList<>();
		...
		if (!wmiComputerSystem.isEmpty()) {
			...
			if (wmiComputerSystemLine.size() > 2) {
				ipmiTable.add(new StringBuilder()
						.append("FRU;")
						.append(wmiComputerSystemLine.get(2))
						.append(HardwareConstants.SEMICOLON)
						.append(wmiComputerSystemLine.get(1))
						.append(HardwareConstants.SEMICOLON)
						.append(wmiComputerSystemLine.get(0))
						.toString());
			}
		}
		...
		StringBuilder deviceSensorList = new StringBuilder();
		...
		for (List<String> line : wmiDiscreteSensors) {
			...
			if (state.substring(0, 16).equals("OEM State,Value=")) {
				// Reverse the bytes of the WORD value
				state = "0x" + state.substring(18, 20) + state.substring(16, 18);
			}

			String entityIdPattern = HardwareConstants.SEMICOLON + entityId + HardwareConstants.SEMICOLON;
			// Is the sensor already a part of the list?
			if (deviceList.toString().contains(entityIdPattern)) {
				// Get the line of this device
				for (int i = 0; i < deviceList.size(); i++) {
					String deviceLine = deviceList.get(i);
					if (deviceLine.contains(entityIdPattern)) {
						// Remove it from the list (we'll then update the line and re-add it to the list)
						deviceList.remove(i);

						// Add the list of states of this sensor to the device
						deviceSensorList = new StringBuilder(deviceLine)
								.append(HardwareConstants.PIPE)
								.append(sensorName)
								.append(HardwareConstants.EQUAL)
								.append(state.replace(HardwareConstants.DOT + HardwareConstants.COMMA, HardwareConstants.PIPE + sensorName + HardwareConstants.EQUAL));

						// Re-add that to the device list
						deviceList.add(deviceSensorList.toString());
						break;
					}
				}
			} else {
				// This is the first time we find this device ID
				deviceSensorList.append(deviceType)
				.append(HardwareConstants.SEMICOLON)
				.append(deviceId)
				.append(HardwareConstants.SEMICOLON)
				.append(entityId)
			...
			for (int i = 0; i < deviceList.size(); i++) {
				String deviceLine = deviceList.get(i);
				if (deviceLine.contains("=Device Removed/Device Absent")) {
					deviceList.remove(i);
					continue;
				}

				// Replace "State Asserted" and "State Deasserted" by 1 and 0
				deviceList.set(i, deviceLine.replace("=State Asserted", "=1")
						.replace("=State Deasserted", "=0")
						.replace("=Deasserted", "=0"));
			}
		}
