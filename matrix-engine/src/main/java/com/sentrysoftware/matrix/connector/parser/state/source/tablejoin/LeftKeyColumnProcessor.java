...
public class LeftKeyColumnProcessor extends TableJoinProcessor {

	private static final Pattern LEFT_KEY_COLUMN_KEY_PATTERN = Pattern.compile(
		"^\\s*((.*)\\.(discovery|collect)\\.source\\(([1-9]\\d*)\\))\\.leftkeycolumn\\s*$",
		Pattern.CASE_INSENSITIVE);

	@Override
	public Matcher getMatcher(String key) {
		return LEFT_KEY_COLUMN_KEY_PATTERN.matcher(key);
	}
	...
	public void parse(final String key, final String value, final Connector connector) {
		...
		tableJoinSource.setLeftKeyColumn(Integer.parseInt(value));
	}
}
