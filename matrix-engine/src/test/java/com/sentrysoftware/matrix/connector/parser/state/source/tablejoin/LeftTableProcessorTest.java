...
class LeftTableProcessorTest {

	private static final String LEFT_TABLE_KEY = "enclosure.discovery.source(3).lefttable";
	...
	void testParse() {
		...
		new LeftTableProcessor().parse(LEFT_TABLE_KEY, VALUE, connector);
		assertEquals(RESULT, tableJoinSource.getLeftTable());
	}
}