...
func TestProcessManager(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, "/bin/sh "+pm.conf.ExecutablePath, cmdline)
...
func TestProcessManagerArgs(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t,
		"/bin/sh "+pm.conf.ExecutablePath+" --server.port 24375",
		cmdline)
}
...
func TestProcessManagerBadExec(t *testing.T) {
	...
	require.Len(t, logObserver.FilterMessage("process died").All(), 2)
}
