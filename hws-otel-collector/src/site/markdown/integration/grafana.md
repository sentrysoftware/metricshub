...
Systems are grouped into sites. This grouping can be easily customized to represent data centers, server rooms, or applications and services. For each site, you can view the consumption in kilowatts per hour, its related cost in dollars, and carbon footprint in metric tons.
...
* **The Power Usage Effectiveness (PUE)**: This is the metric used to determine the energy efficiency of a data center. PUE is determined by dividing the amount of power entering a data center by the power used to run the computer infrastructure within it. The typical PUE in data centers worldwide ranges between 1 to 2.5.
...
The level of temperature is represented with colors from blue (coolest) to red (hottest), allowing you to quickly pinpoint the site, host or device

The *Global* View provides overall comprehensive visibility across your entire monitored environment. It reports on the number of monitored Hosts, their current and forecasted power consumption, electricity cost and carbon emission levels.

   ![Global Information](../images/dashboard_global_info.png)

### Site View

The site grouping can be easily customized to represent data centers, server rooms, or applications and services. For each site, you get the site annual consumption in kiloWattHours, the annual cost in dollars, and the annual carbon footprint in metric tons.

### Identifying the most efficient sites

Based on the information you have provided through the `extraLabels`, you get a classification of the most energy-consuming and highest carbon dioxide emissions sites.
...
This indicates that you might want to consider moving some of your activity to *Ottawa* to save money on your electricity costs and lower your carbon footprint.
...
The most power-consuming devices could be replaced with more efficient ones or moved to a site with lower electricity tariff.
...
The **Host Types** panel shows the composition of your infrastructure distributed among storage, compute and network dedicated devices, while the **Total Power by Host Type** panel displays the electricity consumption and distribution per type.
...
Data centers are energy-intensive facilities. This energy is converted into heat that must be dissipated away from the equipment racks to maintain an optimal room temperature. The hardware devices' temperature is then a critical parameter that needs to be closely monitored.

The **Heating Margin** gauges represent the number of degrees Celsius (°C) remaining before the temperature reaches the closest warning threshold. **${project.name}** collects one day of heating margin measurements for each hardware device and keeps the minimum (critical) value of all the hardware devices in the site.
...
The **Ambient Temperature** and **Hosts Ambient Temperature** panels expose metrics that pinpoint the sites and hosts emitting the less and most heat, while the **Hosts Highest Temperature Sensors** panel shows the hosts with a value collected from the temperature sensors from the coolest to the hottest.

All these indicators can help you optimize your overall data center's ambient temperature by action at the site or the levels. Increasing the ambient temperature in a facility of 1 degree Celsius can make you lower your electricity consumption of 5% and reduce your carbon emission by 5% as well, based on an average PUE of 1.80 (default).
