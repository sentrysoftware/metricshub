keywords: alerts
description: How ${solutionName} triggers alerts and how you can customize the alert's content.
...
**${solutionName}** defines a set of conditions that trigger alerts when failures are detected. These alerts are sent as OpenTelemetry `logs` from the **Hardware Sentry Agent**'s internal `OTLP Exporter` to the **OpenTelemetry Collector**'s internal `OTLP Receiver`.
...
To receive **${solutionName}**'s alerts, your `Exporter` must support the OpenTelemetry `logs` pipeline.
