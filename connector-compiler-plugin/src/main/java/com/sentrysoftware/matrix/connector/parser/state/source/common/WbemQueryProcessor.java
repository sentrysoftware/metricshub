...
import static com.sentrysoftware.matrix.connector.parser.ConnectorParserConstants.SET_WBEM_QUERY;
...
@AllArgsConstructor
public class WbemQueryProcessor extends AbstractStateParser {

	private final Class<? extends Source> type;
	private final String typeValue;

	private static final Pattern WBEM_NAMESPACE_KEY_PATTERN = Pattern
			.compile("^\\s*((.*)\\.(discovery|collect)\\.source\\(([1-9]\\d*)\\))\\.wbemquery\\s*$", Pattern.CASE_INSENSITIVE);
	...
	public void parse(final String key, final String value, final Connector connector) {
		...
		try {
			...
			source.getClass().getMethod(SET_WBEM_QUERY, String.class).invoke(source, value);

		} catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {

			throw new IllegalStateException(
					"WbemQueryProcessor parse: Cannot invoke "
						+ SET_WBEM_QUERY
						+ OPENING_PARENTHESIS
						+ value
						+ CLOSING_PARENTHESIS
